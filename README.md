...
| AR 导览原型 | 摄像头叠加 + 模拟识别 | 预留面向展厅实景导览的交互入口，当前为模拟识别效果 |
...
网页中预留了一个 AR 导览入口，用于展示未来“对准展厅或文物后自动进入对应讲解”的交互方向。当前版本的实现方式是：

- 调用浏览器摄像头权限，打开一个扫描框式的摄像头叠加层。
- 在 2 秒左右的模拟识别后，展示当前展馆或默认展馆的名称与简介。
- 用户可以从 AR 结果入口回到对应展馆页面。

需要注意的是，当前 AR 导览是交互原型，不是完整的真实视觉识别系统；它没有接入图像识别模型，也不会真正判断摄像头前的文物或空间位置。正式展示时可以表述为“AR 导览原型 / 摄像头叠加演示 / 未来视觉识别入口”。
